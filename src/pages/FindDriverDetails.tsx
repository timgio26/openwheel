...
import { distance, getUserLocal } from "../utils/helperFn";
...
export function FindDriverDetails() {
  ...
  const dist = distance(Number(data?.data.origin_lat),Number(data?.data.origin_lng),Number(data?.driverRoute.origin_lat),Number(data?.driverRoute.origin_lng))
  ...
  useEffect(()=>{
    ...
    // if(!selectedDay)navigate('/myroute')
    getPsg()
  },[data,selectedDay])
  ...
  return (
    <div className="h-full flex flex-col justify-between">
      ...
      <div
        className={`border dark:border-0 bg-gray-200 dark:bg-gray-800 text-center py-2 px-4 rounded my-1 ${(data.driverRoute.avail_seat - Number(filledSeat))<1&&'pointer-events-none opacity-50'}`}
